...
int main(int argc, char **argv)
{
    ...
    int student_check = 1;  /* If set, check student info (reset by -a) */
	...
	case 't': /* Directory where the traces are located */
	    ...
	    break;
        case 'a': /* Don't check student info */
            student_check = 0;
            break;
     ...
     */
    if (student_check) {
	/* Students must fill in their information */
	if ((*student.name == '\0') || (*student.id == '\0')) {
	    printf("ERROR.  You must fill in all student information fields!\n");
	    exit(1);
	} 
	else
...
static void usage(void) 
{
    ...
    fprintf(stderr, "\t-a         Don't check the student info.\n");
